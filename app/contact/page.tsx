              ...
              <div className="relative aspect-square bg-parchment overflow-hidden border border-ink/10 shadow-lg shadow-black/10">
                {mapConsent ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center">
                    <p className="text-sm text-stone mb-2">{t("contact.mapConsent") || "Ce contenu est fourni par Google Maps."}</p>
                    <p className="text-xs text-stone/60 mb-4">{t("contact.mapConsentDetail") || "En chargeant la carte, vous acceptez la politique de confidentialité de Google."}</p>
                    <button
                      onClick={() => setMapConsent(true)}
                      className="btn-outline text-xs"
                    >
                      {t("contact.loadMap") || "Charger la carte"}
                    </button>
                  </div>
                )}
              </div>
