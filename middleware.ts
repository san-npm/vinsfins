...
function buildCsp(nonce: string): string {
  return [
    ...
    "img-src 'self' data: blob: https: https://*.zenchef.com https://*.stripe.com",
    "frame-src https://www.google.com https://*.google.com https://*.zenchef.com https://js.stripe.com https://hooks.stripe.com",
    "connect-src 'self' https://sdk.zenchef.com https://*.zenchef.com https://api.stripe.com https://www.google-analytics.com https://*.analytics.google.com https://*.google-analytics.com https://www.googletagmanager.com",
    "object-src 'none'",
    "base-uri 'self'",
...
export const config = {
  ...
  // /admin branch (which sets nonce + CSP) is actually reachable. The /api
  // and asset paths stay excluded — middleware returns NextResponse.next()
  // for them anyway, but skipping the matcher entirely avoids the cold-start
  // hit per request.
  matcher: [
    "/((?!_next|api|favicon\\.ico|robots\\.txt|sitemap\\.xml|og-image|llms\\.txt|images|icons).*)",
  ],
};
